...
Android WiFi 黑名单监测工具 — 连接**安全但不可信**的 WiFi 时通过悬浮窗半透明遮罩警示用户。

> 很多企业/校园网络的通信是加密的（安全），但内部部署了企业 CA 或 TLS 中间人设备，使得特定第三方可以解密你的流量（不可信）。本工具在连接这类网络时以悬浮窗实时提醒。

## 功能

- 🔍 **实时监测**：基于 `ConnectivityManager.NetworkCallback` 事件驱动，零空闲功耗
- ⚫ **黑名单匹配**：支持关键词模糊匹配 SSID（如"公司名"可匹配"Company-Net"）
...
- 连接公司/校园网络时，提醒自己网络流量可能被第三方审计或解密
- 防止手机自动连接到部署了 TLS 中间人的热点
- 对"安全但不可信"的网络保持警惕——加密不代表隐私
